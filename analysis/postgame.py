...
def analyze_final_changes(file_path: str) -> Tuple[List[float], List[float]]:
    ...
    Only considers edits that occurred after the last GlobalEventAdded.
...
def plot_histograms(all_distances: List[float], all_durations: List[float], game_count: int):
    ...
    plt.suptitle(f'Post-Game Edit Analysis - All Games (n={game_count})')
    plt.tight_layout()
    
    # Save the plot
    plt.savefig('final_changes_postgame_all_games.png')
    plt.close()
    
    # Print some basic statistics
    print(f"\nPost-Game Distance Statistics (meters):")
    ...
    print(f"\nPost-Game Duration Statistics (seconds):")
...
if __name__ == "__main__":
    ...
    for filename in os.listdir(SNAPSHOTS_DIR):
        if filename.endswith('.json'):
            ...
            try:
                ...
                if distances:  # Only count games with post-game edits
    ...
    if all_distances:
        ...
        print("No post-game edits found in any games")